...
function Transfer ({ selectedToken, setAction, thirdWebTokens, walletAddress }) {
    ...
    // async function sendCrypto (amount, recipient) {
    ...
    //const sendCrypto = aynsc (amount,recipient) => {
        // if(activeThirdWebToken && amount && recipient) {
        //     const tx = await activeThirdWebToken.transfer(recipient,amount.toString().concat('000000000000000000'))
        //     console.log(tx)
        //     setAction('transferred')
    ...
    return (
        <Wrapper>
            <Amount>
                <FlexInputContainer>
                    <FlexInput placeholder='0' value={amount} onChange={e => setAmount(e.target.value)} />
                    <span>{selectedToken.symbol}</span>
                </FlexInputContainer>
                <Warning style={{color: amount && '#0a0a0d'}}>Amount is a Required Field</Warning>
            </Amount>
            <TransferForm>
                <Row>
